...
11. **Cutover gateway implemented locally 2026-08-19:** KudiRoll can proxy only account, rail-v1 and phase0 JSON routes through `KUDIRAIL_UPSTREAM_URL`; cookies, origin, idempotency and `Set-Cookie` survive, caller authorization is stripped, failures return a no-sign/no-submit `502`, and health fails closed when KudiRail is unavailable. Removing the variable restores the bundled backend while manual passkey/session continuity remains the production gate.
