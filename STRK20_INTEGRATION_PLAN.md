...
- Repository state: public MIT-licensed source at `https://github.com/Cyano88/kudiroll`, green CI, a Railway production candidate, and three verified Mainnet evidence hashes; the public demo video remains pending.
...
2. **E1 — identity and recovery — in progress:** verified email, passkey registration/authentication, encrypted cross-device backup, credential revocation, and recovery ceremony.
   ...
   - **E1d headless-complete 2026-08-19; email delivery activation pending:** the public entry is email-first OTP with enumeration-resistant delivery, keyed code hashes, ten-minute single-use challenges, returning-account discovery and first-time verified-email wallet linking. A new account must create a device passkey before entering; successful registration replaces the bootstrap session with a passkey session. Email can open the workspace but cannot sign transactions, decrypt the embedded wallet, recover funds or delete the account. Activation requires `RESEND_API_KEY`, `KUDIROLL_EMAIL_FROM` and a separate 32+ character `KUDIROLL_EMAIL_CODE_SECRET` in KudiRail.
...
5. **E4 — cutover — early-access presentation corrected 2026-08-30:** Ready X is the explicit live wallet path, while email-first embedded signing is labeled coming soon. Hide extension discovery only after the embedded route produces verified Mainnet evidence; retain an internal recovery/certification build behind deployment access controls.
...
## 10. Phase 3 — atomic payroll — Mainnet no-setup route verified 2026-08-31
...
13. **Public-product hardening deployed 2026-08-27:** code release `c06395b7ff3cb6e2f51cadd805ff3c1df6c1f062` was health-verified against KudiRail `915975fd8899f267a783754273d2b8ecfc6fa1e0`, client custody and PostgreSQL schema 2. KudiRoll now isolates the Starknet Wallet Standard runtime in an asynchronous wallet chunk, reducing the initial production JavaScript from 635.54 kB to 236.43 kB while preserving Ready/Xverse discovery and typed `WalletAccountV6` behavior. KudiRail now publishes a complete secret-safe `.env.example`. Email delivery remains correctly unavailable until real Resend credentials are stored directly in Railway.
14. **Early-access sign-in decision 2026-08-30:** the public entry presents Ready X as the only live first-time wallet path and labels email-first embedded signing as coming soon. The Resend OTP backend remains implemented for the later cutover, but new users are not sent through an email flow that unexpectedly ends at an extension requirement. A saved passkey may reopen an existing workspace; Ready X remains required for private wallet actions until the embedded route is Mainnet-certified.
...
- Production re-verification on 2026-08-27 confirmed both Railway services online, KudiRoll code release `c06395b7ff3cb6e2f51cadd805ff3c1df6c1f062`, KudiRail release `915975fd8899f267a783754273d2b8ecfc6fa1e0`, client custody, and PostgreSQL schema 2. `RESEND_API_KEY` and `KUDIROLL_EMAIL_FROM` remain absent; no placeholder delivery values were introduced.
...
- The 2026-08-19 freshness check found the same Wallet Standard patch drift and shadow-account replacement while stable Wallet API remains 0.10.3. E1d changes authentication only, so the tested wallet pins remain unchanged pending a separate compatibility phase.
