...
| 0:38-0:58 | Payout methods: funded treasury and the completed shield transaction link | "The treasury begins with a separate public USDC shield. After confirmation and the maturity window, the funds are available inside STRK20 for payroll. Separating funding from payment avoids placing the public deposit beside the payroll action." |
...
| 2:36-2:55 | Repository README or launch evidence showing the three transaction links | "The submission includes three verified Mainnet proofs: treasury shielding, an intentional settlement withdrawal, and this atomic two-recipient payroll. KudiRoll is live as an early-access product; Ready X is the current signer, while email-first embedded signing and fully private recipient certification remain clearly tracked next steps." |
| 2:55-3:00 | KudiRoll Home with product URL visible | "KudiRoll: private treasury, one-click team payroll, and accountable payout infrastructure for African businesses." |
