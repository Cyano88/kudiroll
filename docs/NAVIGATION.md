...
Cross-workspace recovery: payment screens identify unresolved payroll and bank orders belonging to the other workspace. Payroll and recorded bank orders link directly to that workspace History; pending bank creation links to its Bank payout recovery controls. Navigation observes existing busy-state guards and does not change account-wide payment safety rules. The production build and all 90 tests passed after this change. Synthetic browser checks verified Enterprise to Personal payroll History and Personal payroll to Enterprise bank recovery; the payroll History destination fitted a 390px viewport with no console errors.
