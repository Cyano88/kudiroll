...
| Private payroll | Ready X STRK20 Wallet API `0.10.3` | A pay run becomes one ordered action list, is simulated, explicitly confirmed, and submitted once. |
...
3. A successful multi-recipient transaction reference produced by the supported privacy wallet.
...
1. Certify two or more registered recipients in one Ready X action list on Mainnet with the smallest safe amounts.
...
3. Deploy the API with managed encrypted persistence, durable sessions, rate limiting, monitoring, backups, and a dedicated RPC.
4. Run wallet compatibility tests for sign-in separately from STRK20 payroll capability tests.
5. Prepare ecosystem listing assets only after the public URL, repository license, privacy policy, and security contact exist.
